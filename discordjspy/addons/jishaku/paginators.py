...
class PaginatorInterface:  # pylint: disable=too-many-instance-attributes
    ...
    async def wait_loop(self):
        ...
        def check(react, react_user):
            ...
            owner_check = (
                react_user.id == self.owner.id if self.owner else not react_user.bot
            )

            return (
                react.message.id == self.message.id
                and react.emoji
                and react.emoji in self.emojis
                and react_user.id != self.bot.user.id
                and owner_check
            )
        ...
        try:
            while not self.bot.is_closed():
                reaction, user = await self.bot.wait_for(
                    "reaction_add", check=check, timeout=3600
                )
